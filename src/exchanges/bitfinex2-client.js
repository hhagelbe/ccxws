...
class Bitfinex2Client extends BasicClient {
  constructor() {
  ...
  _onMessage(raw) {
    ...
    if (channel.channel === "ticker") {
      let market = this._tickerSubs.get(channel.symbol);
      if (!market) return;
    ...
    if (channel.channel === "book" && channel.prec === "R0") {
      let market = this._level3UpdateSubs.get(channel.symbol);
      if (!market) return;
    ...
    if (channel.channel === "book") {
      let market = this._level2UpdateSubs.get(channel.symbol);
      if (!market) return;
  ...
  _onTicker(msg, market) {
    let [bid, bidSize, ask, askSize, change, changePercent, last, volume, high, low] = msg[1];
  ...
  _onTradeMessage(msg, market) {
    let [id, unix, price, amount] = msg[2];
...
module.exports = Bitfinex2Client;
